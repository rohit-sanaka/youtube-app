...
import { getPublishedDuration } from '../utils/helperFuntions'

const VideoCard = ({ info }) => {

  const { contentDetails, snippet, statistics } = info
  const { duration } = contentDetails
  const { channelTitle, publishedAt, localized: { title }, thumbnails: { medium: { url } } } = snippet
  ...
  return (
    <div>
      <div className='relative'>
        ...
        <p className='absolute right-1 bottom-1 px-1 font-semibold text-sm text-white bg-black rounded-md'>{duration1}</p>
      </div>
